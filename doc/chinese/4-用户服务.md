...
#### 2、依赖关系
 ...
 sys-api（用户中心api）、sys-rpc（用户中心rpc）



我们看项目sys/cmd/api/desc/sys.api ，所有的用户api对外的http方法都在这里面
...
我们在写api服务代码的时候是先要在sys.api中定义好service中的方法，然后在desc/user中写request、response，这样拆分开的好处是不那么臃肿
...
2、在app/sys/cmd/api/desc/user/user.api中定义RegisterReq\RegisterResp
...
1）命令行进入app/sys/cmd/api/desc目录下。

2）去项目目录下deploy/script/gencode/gen.sh中，复制如下一条命令，在命令行中执行（命令行要切换到app/sys/cmd/api/desc目录）

```shell
$ goctl api go -api *.api -dir ../  -style=goZero
```

4、打开app/sys/cmd/api/internal/logic/user/register.go文件
...
- 定义protobuf文件
  ...
  我们在app/sys/cmd/rpc/pb中新建sys.proto，写入注册方法
...
- 使用goctl生成代码，这里不需要自己手动敲
  ...
  1）命令行进入app/sys/cmd/rpc/pb目录下。

  2）去项目目录下deploy/script/gencode/gen.sh中，复制如下两条命令，在命令行中执行（命令行要切换到app/sys/cmd/rpc/pb目录）
...
- 打开app/sys/cmd/rpc/internal/logic/registerLogic.go写逻辑代码
...
在文件aiops/common/ctxdata/ctxData.go
