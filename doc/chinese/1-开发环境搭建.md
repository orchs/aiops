...
![gozeroaiops](./images/1/gozeroaiops.png)

### 四、业务架构图
![gozeroaiops](./images/1/aiops-service.png)
...
```shell
$ git clone https://github.com/Mikaelemmmm/aiops
$ go mod tidy
```
...
```shell
$ docker network create aiops_aiops_net
...
```shell
$ ./kafka-topics.sh --create --zookeeper zookeeper:2181 --replication-factor 1 -partitions 1 --topic aiops-log
...
```mysql
create database aiops_order charset = utf8;
create database aiops_payment charset = utf8;
create database aiops_travel charset = utf8;
create database aiops_sys charset = utf8;
```

use aiops_order;
导入deploy/sql/aiops_order.sql数据

use aiops_payment;
导入deploy/sql/aiops_payment.sql数据

use aiops_travel;
导入deploy/sql/aiops_travel.sql数据
...
```shell
$ curl  -X POST "http://127.0.0.1:8888/sys/v1/user/register" -H "Content-Type: application/json" -d "{\"mobile\":\"18888888888\",\"password\":\"123456\"}" 
...
然后在当前页面，Create index pattern->输入aiops-*  -> Next Step ->选择@timestamp->Create index pattern
...
- kafka中没有创建log的topic ： aiops-log

  解决：去kafka创建 aiops-log，重启filebeat、go-stash
...
- 内部kafka问题
  ...
  $ docker exec -it kafka /bin/sh
  $ cd /opt/kafka/bin
  $ ./kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic aiops-log 
  ```
  ...
  ​		  2）在kafka容器内部命令行使用consumer.sh消费aiops-log，另外一个终端命令行用producer.sh给aiops-log发送消息，如果consumer收不到，说明kafka出问题了，docker logs -f kafka看看什么问题
...
一般我们在生成api，rpc代码时候手动去敲goctl的命令比较长，也记不住，所以我们直接去deploy/script/gencode/gen.sh中复制代码即可。比如我在sys服务中新增加了一个业务，修改密码，写完api文件之后，进入到sys/cmd/api/desc目录下，直接复制deploy/script/gencode/gen.sh中的生成api命令运行即可
