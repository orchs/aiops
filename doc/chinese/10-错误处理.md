...
那么我们想一下，如果有一个统一的地方记录日志，同时在业务代码中只需要一个return err 就能将返回给前端的错误提示信息、日志记录相信信息分开提示跟记录，如果按照这个思路实现，那简直不要太爽，是的 aiops就是这么处理的，接下来我们看下。
...
import (
	...
	"aiops/app/identity/cmd/rpc/identity"
...
func (l *RegisterLogic) Register(req types.RegisterReq) (*types.RegisterResp, error) {
	registerResp, err := l.svcCtx.UsercenterRpc.Register(l.ctx, &sys.RegisterReq{
		Mobile:   req.Mobile,
...
接下来我们看最终返回给前端怎么处理的，我们接着看app/sys/cmd/api/internal/handler/user/registerHandler.go
...
这里可以看到，aiops生成的handler代码 有2个地方跟默认官方的goctl生成的代码不一样，就是在处理错误处理的时候，这里替换成我们自己的错误处理了，在common/result/httpResult.go 
