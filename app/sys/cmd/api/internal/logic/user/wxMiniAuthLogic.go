...
func (l *WxMiniAuthLogic) WxMiniAuth(req types.WXMiniAuthReq) (*types.WXMiniAuthResp, error) {
	...
	var userId int64
	rpcRsp, err := l.svcCtx.SysRpc.GetUserAuthByAuthKey(l.ctx, &sys.GetUserAuthByAuthKeyReq{
	...
	if rpcRsp.UserAuth == nil || rpcRsp.UserAuth.Id == 0 {
		...
		nickName := fmt.Sprintf("aiops%s", mobile[7:])
		registerRsp, err := l.svcCtx.SysRpc.Register(l.ctx, &sys.RegisterReq{
			AuthKey:  authResult.OpenID,
			AuthType: sysModel.UserAuthTypeSmallWX,
			Mobile:   mobile,
			Nickname: nickName,
		...
		if err != nil {
			return nil, errors.Wrapf(ErrWxMiniAuthFailError, "SysRpc.Register err :%v, authResult : %+v", err, authResult)
		}
		...
		tokenResp, err := l.svcCtx.SysRpc.GenerateToken(l.ctx, &sys.GenerateTokenReq{
			UserId: userId,
		})
		if err != nil {
			return nil, errors.Wrapf(ErrWxMiniAuthFailError, "SysRpc.GenerateToken err :%v, userId : %d", err, userId)
		}
		return &types.WXMiniAuthResp{
